...
class Tree(treestatus_api.lib.db.db.Model):
    ...
    def to_dict(self):
        return dict(tree=self.tree, status=self.status, reason=self.reason, message_of_the_day=self.message_of_the_day)
...
class Log(treestatus_api.lib.db.db.Model):
    ...
    def to_dict(self):
        return dict(id=self.id, tree=self.tree, when=self.when, who=self.who, status=self.status, reason=self.reason, tags=self.tags)
...
class StatusChange(treestatus_api.lib.db.db.Model):
    ...
    def to_dict(self):
        return dict(trees=[t.to_dict() for t in self.trees], status=self.status, when=self.when, who=self.who, reason=self.reason, id=self.id)
...
class StatusChangeTree(treestatus_api.lib.db.db.Model):
    ...
    def to_dict(self):
        return dict(tree=self.tree, last_state=load_last_state(self.last_state), id=self.id)
