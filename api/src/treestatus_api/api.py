...
import treestatus_api.lib.auth
import treestatus_api.lib.cache
...
log = treestatus_api.lib.log.get_logger(__name__)


CombinedTree = collections.namedtuple("CombinedTree", ["tree", "message_of_the_day", "tags", "status", "reason", "log_id", "model"])
...
def _statuspage_data(resolved, component_id, tree, status_from, status_to):
    data = {"status": resolved and "resolved" or "investigating", "components": {component_id: resolved and "operational" or "major_outage"}}
    if not resolved:
        data["name"] = f"Tree {tree.tree} closed"
        data["component_ids"] = [component_id]
        data["metadata"] = {"treestatus": {"tree": tree.tree, "status_from": status_from, "status_to": status_to}}
        data["body"] = f"Message of the day: {tree.message_of_the_day}\n" f"Reason: {tree.reason}\n"
...
def _statuspage_send_email_on_error(subject, content, incident_id=None):
    ...
    link = {"href": f"https://manage.statuspage.io/pages/{page_id}", "text": "Visit statuspage"}
    if incident_id:
        link = {"href": f"https://manage.statuspage.io/pages/{page_id}/incidents/{incident_id}", "text": "Visit statuspage incident"}
    flask.current_app.notify.email({"address": address, "subject": subject, "content": content, "link": link})


def _statuspage_create_incident(headers, component_id, tree, status_from, status_to):
    page_id = flask.current_app.config.get("STATUSPAGE_PAGE_ID")
    if not page_id:
        log.error("STATUSPAGE_PAGE_ID not defined in app config.")
        return

    data = _statuspage_data(False, component_id, tree, status_from, status_to)
    log.debug(f"Create statuspage incident for tree `{tree.tree}` under page `{page_id}`", data=data)
    response = requests.post(f"{STATUSPAGE_URL}/pages/{page_id}/incidents", headers=headers, json=data)
    try:
...
def _statuspage_resolve_incident(headers, component_id, tree, status_from, status_to):
    page_id = flask.current_app.config.get("STATUSPAGE_PAGE_ID")
    response = requests.get(f"{STATUSPAGE_URL}/pages/{page_id}/incidents/unresolved", headers=headers)
    try:
        response.raise_for_status()
    except Exception as e:
        log.exception(e)
    ...
    for incident in incidents:
        if (
            "id" in incident
            and "metadata" in incident
            and "treestatus" in incident["metadata"]
            and "tree" in incident["metadata"]["treestatus"]
            and incident["metadata"]["treestatus"]["tree"] == tree.tree
        ):
    ...
    response = requests.patch(
        f"{STATUSPAGE_URL}/pages/{page_id}/incidents/{incident_id}", headers=headers, json=_statuspage_data(True, component_id, tree, status_from, status_to)
    )
    try:
        response.raise_for_status()
    except Exception as e:
...
def _notify_status_change(trees_changes):
    if flask.current_app.config.get("STATUSPAGE_ENABLE"):
        ...
        if not token:
            ...
            for tree_change in trees_changes:
                ...
                # create an accident
                if status_from in ["open", "approval required"] and status_to == "closed" and tag in statuspage_tags:
                    _statuspage_create_incident(headers, component_id, tree, status_from, status_to)

                # close an accident
                elif status_from == "closed" and status_to in ["open", "approval required"]:
                    _statuspage_resolve_incident(headers, component_id, tree, status_from, status_to)
    ...
    if flask.current_app.config.get("PULSE_TREESTATUS_ENABLE"):
        ...
        for tree_change in trees_changes:
            ...
            payload = {"status_from": status_from, "status_to": status_to, "tree": tree.to_dict(), "tags": tags}
            routing_key = routing_key_pattern.format(tree.tree)

            log.info("Sending pulse to {} for tree: {}".format(exchange, tree.tree))
...
def _update_tree_status(session, tree, status=None, reason=None, tags=[], message_of_the_day=None):
    ...
    if status or reason:
        ...
        if reason is None:
            reason = "no change"
        log = treestatus_api.models.Log(tree=tree.tree, when=_now(), who=flask_login.current_user.get_id(), status=status, reason=reason, tags=tags)
        session.add(log)

    treestatus_api.lib.cache.cache.delete_memoized(_get_tree, tree.tree)


@treestatus_api.lib.cache.cache.memoize()
def _get_tree(tree):
    ...
    q = q.add_columns(treestatus_api.models.Log._tags, treestatus_api.models.Log.status, treestatus_api.models.Log.reason, treestatus_api.models.Log.id)
    q = q.outerjoin(treestatus_api.models.Log, treestatus_api.models.Log.tree == treestatus_api.models.Tree.tree)
    q = q.order_by(treestatus_api.models.Tree.tree.desc(), treestatus_api.models.Log.when.desc())
...
@treestatus_api.lib.auth.auth.require_permissions([treestatus_api.config.SCOPE_TREES_UPDATE])
...
def update_trees(body):
    ...
    if _is_unset(body, "tags") and _get(body, "status") == "closed":
    ...
    if not _is_unset(body, "remember") and body["remember"] is True:
        if _is_unset(body, "status") or _is_unset(body, "reason") or _is_unset(body, "tags"):
            raise werkzeug.exceptions.BadRequest("must specify status, reason and tags to remember the change")
    ...
    for tree in trees:
        ...
        _update_tree_status(session, tree.model, status=new_status, reason=new_reason, message_of_the_day=new_motd, tags=new_tags)
    ...
    if not _is_unset(body, "remember") and body["remember"] is True:
        ...
        ch = treestatus_api.models.StatusChange(who=flask_login.current_user.get_id(), reason=body["reason"], when=_now(), status=body["status"])
...
@treestatus_api.lib.auth.auth.require_permissions([treestatus_api.config.SCOPE_TREES_CREATE])
...
def _kill_tree(tree):
    ...
    session.commit()
    treestatus_api.lib.cache.cache.delete_memoized(_get_tree, tree)


@treestatus_api.lib.auth.auth.require_permissions([treestatus_api.config.SCOPE_TREES_DELETE])
def kill_tree(tree):
    _kill_tree(tree)
    return None, 204
...
def get_stack():
    return dict(result=[i.to_dict() for i in treestatus_api.models.StatusChange.query.order_by(treestatus_api.models.StatusChange.when.desc())])


@treestatus_api.lib.auth.auth.require_permissions([treestatus_api.config.SCOPE_REVERT_CHANGES])
def update_stack(id, body=dict()):
    ...
    for tree in change.trees:
        ...
        _update_tree_log(last_state["current_log_id"], last_state["current_tags"], last_state["current_reason"])
        tree.last_state = json.dumps(last_state)
...
def _revert_change(id, revert=None):
    ...
    if revert:
        for chtree in ch.trees:
            ...
            _update_tree_status(session, tree.model, status=last_state["status"], reason=last_state["reason"], tags=last_state.get("tags", []))

            if last_state["status"] and tree.status != last_state["status"]:
                trees_status_change.append((tree, tree.status, last_state["status"], last_state.get("tags", [])))
