...
EXTENSIONS = ["log", "security", "cors", "api", "auth", "pulse", "db", "cache"]
...
def create_app(project_name, extensions=[], config=None, enable_dockerflow=True, **kw):
    ...
    for extension_name in EXTENSIONS:
        ...
        try:
            extension_init_app = getattr(importlib.import_module("treestatus_api.lib." + extension_name), "init_app")
        except Exception as e:
            logger.exception(e)
            pass

        if extension_init_app is None:
            raise Exception(f"Could not import treestatus_api.lib extension: {extension_name}")
    ...
    if enable_dockerflow:
        app.add_url_rule("/__heartbeat__", view_func=treestatus_api.lib.dockerflow.heartbeat_response)
        app.add_url_rule("/__lbheartbeat__", view_func=treestatus_api.lib.dockerflow.lbheartbeat_response)
        app.add_url_rule("/__version__", view_func=treestatus_api.lib.dockerflow.get_version)
