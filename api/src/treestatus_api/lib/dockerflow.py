...
def get_version():
    version_json = {"source": "https://github.com/mozilla-releng/treestatus", "version": "unknown", "commit": "unknown", "build": "unknown"}
    try:
...
def heartbeat_response():
    ...
    for extension_name in extensions:
        if extension_name not in treestatus_api.lib.flask.EXTENSIONS:
            continue

        extension_heartbeat = None
        try:
            app_heartbeat = getattr(importlib.import_module("treestatus_api.lib.flask." + extension_name), "app_heartbeat")
